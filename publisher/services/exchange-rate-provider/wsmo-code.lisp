...
(eval-when (eval load)
  (setf (gethash '(us-dollar euro) *currency-rates*) 0.795765 
        (gethash '(us-dollar pound) *currency-rates*) 0.547965 
        (gethash '(us-dollar lira) *currency-rates*) 1565.51
        (gethash '(us-dollar yen) *currency-rates*) 109.171
        (gethash '(us-dollar rupee) *currency-rates*) 43.5150 
...
(defun exchange-rate-provider-fun3 (source-currency target-currency)
  ...
  (exchange-rate-provider-fun2 source-currency target-currency))
...
(eval-when (eval load)
  (irs-wsmo-web-service-registration wsmo-exchange-rate ;;this is the ontology name
  ...
  (irs-wsmo-web-service-registration currency-converter ;;this is the ontology name
