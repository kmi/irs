...
(defun in-region-p (city-or-town region)
  (ocml::holds? 'ocml::is-in-geographical-region (make-ocml-symbol city-or-town)
  ...
  (destructuring-bind (seconds minutes hours date month year) list-time-and-date
    (let ((train-time (first-train-after departure hours minutes seconds)))
      (format nil "~a: ~a is booked on the ~d going from ~a to ~a at ~d:~d, ~d, ~a ~d.~%The price is ~d Euros."
              name person (random 1000) departure destination
    ...
    (if host-country
        (destructuring-bind (start interval)
          ...
          (format nil "Timetable of trains from ~a to ~a on ~d, ~a, ~d~{~%~{~d:~d~}~}"
                  departure destination (car date) (second date) (third date)
...
(book-german-train-journey 'john 'berlin 'frankfurt (encode-universal-time 3 4 6 18 8 2004))

(book-german-train-journey 'john 'berlin 'paris (encode-universal-time 3 4 6 18 8 2004))

(book-french-train-journey 'john 'paris 'lyon (list 3 4 6 18 8 2004))

(book-student-european-train-journey 'john 'paris 'innsbruck (list 3 4 6 18 8 2004))
