...
(def-class shopping-item ()
  ((has-normal-price :type real-number :min-cardinality 1)
   ...
   (has-special-offer :type boolean :cardinality 1)
...
(def-class customer (affiliated-person)
  ((customer-id :type string :cardinality 1)
   ...
   (user-name :type string :cardinality 1)
   (is-of-customer-type :type customer-type)))
...
(def-function all-item-names-in-category (?cat) -> ?names
  :body
  (setofall ?n (and (instance-of ?x ?cat)
                    (item-name ?x ?n))))
...
;;;; Application and Task(s) for classification (put in separate file later)
;
;;; TASK
;(def-instance single-classification-1 single-solution-classification-task
;  ((has-input-role )
;   (has-output-role )
;   (has-candidate-solutions (customer))
;   (has-observables )
;   (has-match-criterion  abstraction-aware-match-criterion)
...
;
;;; PSM
;(def-instance classification-method-1 heuristic-admissible-sol-classifier
;  ((has-precondition )
;   (has-goal-expression )
;   (has-control-role )
;   (tackles-task )
...
;   (has-output-role )
;   (has-abstractors )
;   (has-refiners  (refinement-through-subclass-of-links))
;   (has-observables )
;   (has-solution-exclusion-criterion  default-candidate-exclusion-criterion)
...
;
;;; APPLICATION
;(def-instance customer-classification application
;  ((tackles-domain personalized-e-commerce)
;   (uses-method  classification-method-1)
;   (tackles-task  single-classification-1)))
