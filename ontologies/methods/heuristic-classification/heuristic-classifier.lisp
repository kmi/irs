...
(def-class heuristic-admissible-sol-classifier (decomposition-method) ?psm
            ...
            (in-environment 
             ...
             (loop for ?candidate in ?ranked-candidates
                   ...
                   (if (not (ruled-out-solution ?candidate 
                                                (apply-match-criterion ?match-criterion
                                                                       ?obs
                                                                       ?candidate)
                                                ?exclusion-criterion))
                     (in-environment 
...
(def-class heuristic-optimal-sol-classifier (decomposition-method) ?psm
  ...
  ((has-input-role :value has-abstractors
   ...
   (has-body 
    :value 
    '(lambda (?psm)
       ( heuristic-optimal-sol-classifier ?psm))))
...
(def-procedure heuristic-optimal-sol-classifier (?psm)
  ;;;;:lisp-fun #'heuristic-optimal-sol-classifier
  :body
       (in-environment                                              
        ((?obs . (achieve-generic-subtask 
                  ?psm abstraction
         ...
         (if (null ?good-refined-sols)
           ...
           (in-environment 
            ...
            (complete-optimal-search ?psm ?obs ?refs 
                                     ...
                                     ?admissibility-criterion
                                     ?exclusion-criterion))))
 ;;; :lisp-fun #'complete-optimal-search
  )
...
(def-relation directly-abstracts-from (?ob ?obs)
  "Use this relation to represent direct abstraction steps"
  :constraint (and (observable ?ob)
                   (observables ?obs)))
(def-class abstractors (set) ?l
  :iff-def (every ?l abstractor)
  :prove-by (= ?l (setofall ?x (abstractor ?X)))
   :no-proofs-by (:iff-def))


(def-class abstractor (unary-function) 
  ((domain :value observables)
   (range :value observable)
   (applicability-check :type abstractor-applicability-condition-class)))
...
(def-relation abstractor-is-applicable? (?ab ?observables)
  ...
  :iff-def (holds (the ?appl (applicability-check ?ab ?appl)) ?observables))
    ...
    :value (kappa (?task ?observables)
                  (and (forall ?ob
                               (=> (member ?ob ?observables)
                                   (or (member ?ob (role-value ?task has-observables))
...
(def-class abstraction-psm (decomposition-method) ?psm
  ...
  :own-slots ((tackles-task-type 'abstraction)
              (has-generic-subtasks '(one-step-abstraction 
                                      select-abstractor
                                      ))))
...
(def-class select-abstractor (goal-specification-task) ?task
  ((has-input-role :value has-observables
   ...
   (has-observables :type observables)
   (has-abstractors  :type list)
   (has-abstractor :type abstractor-or-nothing)
   (has-goal-expression 
    :value 
...
(def-class abstractor-or-nothing () ?x
  :iff-def (or (abstractor ?X)
               (= ?x :nothing)))
...
;;;INSTANCE abstraction-aware-scoring-mechanism
       ...
       (loop for feature in obs-features
          ...
          (progn 
            ...
            (setf (third score-vector) (set-difference (third score-vector) 
                                                       (ocml-eval-gen `(all-explained-features ',(second score-vector) ',observables))))              (return score-vector)))))
...
(def-instance abstraction-aware-macro-score-mechanism macro-score-mechanism)


(def-function  all-explained-features (?features ?observables)
  "We assume that the features in ?features are explained and 
  want to find all features that are in the tree rooted at a feature in ?features"
  :constraint (every ?features feature) 
  :body (setofall ?feature (exists (?ob ?f ?obs) 
                                   (and (member ?ob ?observables)
                                        (has-observable-feature ?ob ?f)
                                        (member ?f ?features)
                                        (directly-abstracts-from ?ob ?obs)
                                        (member ?feature (all-features-in-observables ?obs))))))
...
(def-class  rank-solutions-psm (decomposition-method) ?psm
...
;;;;;;;;;;REFINEMENT TASK;;;;;;;;;;;;;;;;;;;;;;;;;
(def-class refiners (set) ?l
  :iff-def (every ?l refiner)
  :prove-by (= ?l (setofall ?x (refiner ?X)))
   :no-proofs-by (:iff-def))

;;;CLASS REFINER
(def-class refiner (unary-function) 
  ((domain :value solution)
   (range :value solution-space)
   (applicability-check :type refiner-applicability-condition-class)))

(def-class refiner-applicability-condition-class () ?x
  "The applicability condition for a refiner must be a relation whose domain is 
    a subset of class solution"
  :iff-def (and (unary-relation ?x)
                (subset (domain ?x) solution))
  :sufficient-for-type-checking (unary-relation ?x)
   :no-proofs-by (:iff-def))

(def-relation refiner-is-applicable? (?ref ?solution)
  :constraint (and (solution ?solution)
                   (refiner ?ref))
  :iff-def (holds (the ?appl (applicability-check ?ref ?appl)) ?solution))
...
(defun heuristic-classify (&key (solution-space 
                                 (setofall '?x '(solution ?x)))
                                 observables abstractors refiners 
                                domain-name
...
;;;;;;;;;;;;;;;;;;;;;
(def-class non-hierarchical-optimal-classifier (PRIMITIVE-METHOD) ?psm
  ...
  ((has-input-role 
   ...
   (has-body 
    :value 
    '(lambda (?psm)
       ( non-hierarchical-optimal-classifier  ?psm))))
  
  :own-slots ((tackles-task-type optimal-classification-task)
              ))

(defun non-hierarchical-optimal-classifier (?psm)
  ;;we assume 
  ;; - obs do not change
  ;; - no abstraction mechanisms
  ;; - no refinement mechanisms
  ;; - no domain hierarchy
  
  (let*                                              
    ((?obs  (role-value ?psm 'has-observables))
     (all-candidates (role-value ?psm 'has-candidate-solutions))
     (sol-admissibility-criterion (role-value ?psm 
                                              'has-solution-admissibility-criterion))
     (match-criterion (role-value 
                       ?psm 'has-match-criterion))
     (sol-pairs (filter (mapcar #'(lambda (candidate)
                                    (cons candidate 
                                          (ocml-eval-gen `(apply-match-criterion 
                                                           ',match-criterion
                                                           ',?obs
                                                           ',candidate))))
                                all-candidates)
                        #'(lambda (pair)
                            (holds? 'admissible-solution  
                                    (car pair) ;;candidate
                                    (cdr pair) ;;score
                                    sol-admissibility-criterion))))
     (sorted-sol-pairs (sort sol-pairs
                             #'(lambda (?pair1 ?pair2)
                                 (holds? 'better-match-score  (cdr ?pair1)(cdr ?pair2)
                                         match-criterion)))))
   
    (when sorted-sol-pairs
           (loop with first = (caar sorted-sol-pairs)
                 with sols = (List first)
                 for other-pair in (cdr sorted-sol-pairs)
                 until  (holds? 'better-match-score  
                                (cdr (car sorted-sol-pairs))
                                (cdr other-pair)
                                match-criterion)
                 do
                 (push (car other-pair) sols)
                 finally
                 (return sols)))))


(def-procedure non-hierarchical-optimal-classifier (?psm)
  :lisp-fun #'(lambda (x) (non-hierarchical-optimal-classifier x)))

(defun flat-optimal-classify (&key (solution-space 
                                 (setofall '?x '(solution ?x)))
                                 observables   
                                domain-name
                                (task-type 'OPTIMAL-CLASSIFICATION-TASK)
                                (match-criterion 'default-match-criterion)
                                (method-type 'non-hierarchical-optimal-classifier)
                                (solution-admissibility-criterion 
                                 'default-solution-admissibility-criterion)
                                )
