...
(defun html-describe-item (stream name type &optional
                                  (lookup-function
  ...
  (let ((output (with-output-to-string (stream)
                  ...
                  (handler-case 
                      (case type
                        ((:class) (html-describe-class stream name lookup-function))
                        ((:instance) (html-describe-instance stream name :lookup-function lookup-function))
                        ((:relation) (html-describe-relation stream name lookup-function))
                        ((:function) (html-describe-function stream name lookup-function))
                        ((:ontology)
                         (select-ontology name) 
                         (html-describe-ontology stream name lookup-function))
                        ((:unknown) 
                         (html-describe-unknown-item stream name lookup-function))
                        (t (format stream "Unknown type to describe ~a." 
                                   type)))
                    (error (c) (format stream "Sorry, had a problem displaying ~(~a~)." name))
                    (serious-condition (c) (format stream "Sorry, had a problem displaying ~(~a~)." name)))
                  (format stream "</body></html>~%"))))
    (format stream "~a~%"
            (remove #\linefeed (remove #\return output)))))


(defun html-describe-unknown-item (stream name &optional
                                          (lookup-function
                                           #'mnm-lookup-current-word))
  ;;(setf nn name ll lookup-function)
  (cond ((get-ocml-class name)
         (html-describe-class stream name lookup-function))
        ((get-ocml-relation name)
         (html-describe-relation stream name lookup-function))
        ((get-ocml-function name)
         (html-describe-function stream name lookup-function))
        ((get-ontology name)
         (select-ontology name)
         (html-describe-ontology stream name lookup-function))
        ((find-current-instance name)
         (html-describe-instance stream name :lookup-function lookup-function))
  ...
  (let ((domain-slots (domain-slots class)))
    (when domain-slots
      ...
      (loop for slot in domain-slots
            do
            (describe-slot slot class ontology stream lookup-function)))
    (let ((own-slots (own-slots class)))
      (when own-slots
        (format stream "<br>~A" (http::bold "Own Slots:"))
        (loop for slot-name-and-value in own-slots
              do
              (describe-own-slot slot-name-and-value ontology stream lookup-function))))
    (let ((instances 
           (mapcar 
            #'(lambda (x)
      ...
      (when instances
        ...
        (format stream 
                (http::internal-insert-ocml-links (format nil "~(~{~a ~}~)"
                                                          instances)
                                                  lookup-function ontology))))))

#|
don't have to look up and insert ocml links
                (http::internal-insert-ocml-links
                 (format nil "~(~{~a ~}~)"
                         instances)
                 lookup-function
...
(defun html-describe-instance (stream name &key class-name deduce-all-values? 
  ...
  (Let ((inst (find-current-instance name class-name))
    ...
    (when inst
      ...
      (format stream 
              (http::internal-insert-ocml-links 
               (format nil "<h2>Instance ~:(~a~) of class ~:(~a~)</h2>"
                       name (name parent))
               lookup-function
...
(defun html-describe-relation (stream name &optional
                                   (lookup-function
                                    #'mnm-lookup-current-word))
  (Let* ((rel (get-relation name)))
    (if rel
        (html-describe-relation-internal stream name rel lookup-function)
      (format stream "~:(~a~) is not a relation." name))))

(defun html-describe-relation-internal (stream name rel &optional  (lookup-function
                                                                    #'mnm-lookup-current-word))
  (format stream 
          (http::internal-insert-ocml-links
           (format nil "<h2>Relation ~:(~a~)</h2>" name)
           lookup-function
           (name *current-ontology*)))
  (format stream 
          (http::internal-insert-ocml-links
           (format nil "<b>Schema:</b> ~:(~a~)" (schema rel))
           lookup-function
           (name *current-ontology*)))
  (format stream 
          (http::internal-insert-ocml-links
           (format nil "<br><b>Ontology:</b> ~:(~a~)"
  ...
  (when (ocml-documentation rel)
    (format stream
            (http::internal-insert-ocml-links
             (format nil "<br><b>Documentation:</b> <i>~:(~a~)</i>" 
                     (ocml-documentation rel))
             lookup-function
             (name *current-ontology*))))
  (when (local-slot-of rel)
    (let ((local-slot-of (remove-duplicates (mapcar #'name (local-slot-of rel)))))
  ...
  (when (slot-of rel)
    (Let ((non-local-slots 
           (set-difference (remove-duplicates (mapcar #'name (slot-of rel)))
                           (remove-duplicates (mapcar #'name (local-slot-of rel))))))
...
(defun html-describe-function (stream name &optional
                                   (lookup-function
                                    #'mnm-lookup-current-word))
  (Let* ((f (get-function name)))
    (if f
        (html-describe-function-internal stream name f lookup-function)
      (format stream "~:(~a~) is not a function." name))))

(defun html-describe-function-internal (stream name f &optional
                                               (lookup-function
                                                #'mnm-lookup-current-word))
  (format stream 
          (http::internal-insert-ocml-links
           (format nil "<h2>Function ~:(~a~)</h2>"  name (schema f))
           lookup-function
           (name *current-ontology*)))
  (format stream 
          (http::internal-insert-ocml-links
           (format nil "<b>Schema</b>: ~:(~a~)" (schema f))
           lookup-function
           (name *current-ontology*)))
  (format stream
          (http::internal-insert-ocml-links
...
(defun html-describe-ontology (stream name &optional
                                      (lookup-function
                                       #'mnm-lookup-current-word))
  "If deduce-all-values? is T then we use all inference methods
    at our disposal to find slot values"
